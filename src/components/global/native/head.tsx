...
const Header = (props: DefaultMetaDataProps) => {
  ...
  const canonicalUrl: string = (`https://phorfolio.site` + (router.asPath === '/' ? '' : router.asPath)).split('?')[0];
  const metaInfoTags: MetaInfoTagsProps[] = [
    ...
    { name: 'og:url', content: canonicalUrl || `https://phorfolio.site` },
    { name: 'og:description',content: props?.description || defaultMetaData.description},
    { name: 'og:site_name', content: 'Phorfolio' },
